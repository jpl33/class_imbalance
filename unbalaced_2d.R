...
levels(ub_y)<-c(0,1)
# xfd_ub_smt<-ubSMOTE(xfd[,1:2],ub_y, perc.over = 0.5*100*(nrow(r1)/nrow(r2)), k = 5, perc.under = 100, verbose = TRUE)
...
# smote1<-ggplot(xfd_ub_smt, aes(x=X1,y=X2,colour=src))+geom_point(aes(size=4, shape = 21))+geom_text(aes(label=rownames(xfd_ub_smt)))+scale_shape_identity()
row.names(xfd_smt)<-seq(1,nrow(xfd_smt))
smote1<-ggplot(xfd_smt, aes(x=X1,y=X2,colour=src))+geom_point(aes(size=4, shape = 21))+geom_text(aes(label=rownames(xfd_smt)))+scale_shape_identity()+ggtitle("smote")



task_smt<-makeClassifTask(data = xfd_smt, target = "src",positive = "r2")
lrn_rf<-makeLearner("classif.randomForest",predict.type = "prob")
...
rose<-ggplot(xfd_rose, aes(x=X1,y=X2,colour=src))+geom_point(aes(size=4, shape = 21))+geom_text(aes(label=rownames(xfd_rose)))+scale_shape_identity()+ggtitle("rose")
...
tomek_smt<-TomekClassif(src~.,xfd_smt,Cl = "all", rem = "both")
xfd_smt2<-tomek_smt[[1]]

adapted_tomek<-function(df){
tomek_smt<-TomekClassif(src~.,df)
# find all Tomek link points
zsmt<-xfd_smt2[tomek_smt[[2]] ,]
# get the tomek link points from the minority class
tomek_min_smt<-as.numeric(row.names( zsmt[zsmt$src=="r2",]))
knn_qry_smt<-xfd_smt2[tomek_min_smt,]
# get the t nearest neighbors for the minority tomek link points
tomek_knn_smt<-get.knnx(xfd_smt2[,1:2],knn_qry_smt[,1:2],k=t)
...
for (i in 1:nrow(tomek_knn_smt$nn.index)){
  ...
  for (j in 2:t){
    # for each point, look up if its t nearest neighbors are  minority or majority
    # if it's a minority- raise the index, if it's a majority - leave index at "0"
    rm_id<-ifelse(xfd_smt2[tomek_knn_smt$nn.index[i,1],"src"]==xfd_smt2[tomek_knn_smt$nn.index[i,j],"src"],rm_id<-rm_id+1,rm_id)
  }
  # if rm_id is less than 2, most neighbors around the minority points are majority!
...
if (nrow(tomek_knn_smt$nn.index[tomek_knn_smt$nn.index[,t+1]==1,])>0){
   xfd_smt2<-xfd_smt2[-(tomek_knn_smt$nn.index[tomek_knn_smt$nn.index[,t+1]==1,1]),]
}

if (nrow(tomek_knn_smt$nn.index[tomek_knn_smt$nn.index[,t+1]==0,])>0){
  tomek_maj<-xfd_smt2[rownames(xfd_smt2) %in% (tomek_knn_smt$nn.index[tomek_knn_smt$nn.index[,t+1]==0,2]),]
  xfd_smt2<-xfd_smt2[-as.numeric(row.names(tomek_maj)),]
}

}
smote_tomek<-ggplot(xfd_smt2, aes(x=X1,y=X2,colour=src))+geom_point(aes(size=4, shape = 21))+geom_text(aes(label=rownames(xfd_smt2)))+scale_shape_identity()+ggtitle("smote_tomek")

task_smt_tomek<-makeClassifTask(data = xfd_smt2, target = "src",positive = "r2")
train_smt_tomek<-train(lrn_rf,task_smt_tomek)
predict_smt_tomek<-predict(train_smt_tomek,newdata = xft)

tomek_rose<-TomekClassif(src~.,xfd_rose,Cl = "all", rem = "both")
...
grid.arrange(orig,smote1,rose,smote_tomek, rose_tomek, ncol=2)
...
roc$data %>% group_by(learner) %>%
  summarise(mauc=max(auc),mgmean=max(gmean)) -> result