...
X1<- genPositiveDefMat(4,lambdaLow=10)
X2<- genPositiveDefMat(4,lambdaLow=4)
...
r1<-mvrnorm(200,c(10,8,9,6),as.matrix(X1$Sigma))
r2<-mvrnorm(20,c(6,7,5,7),as.matrix(X2$Sigma))
...
xfd<-data.frame(X1=XX[,1], X2=XX[,2], X3=XX[,3],X4=XX[,4],src=as.factor(XX[,5]))
...
r3<-mvrnorm(200,c(10,8,11,7),as.matrix(X1$Sigma))
r4<-mvrnorm(20,c(6,7,4,5),as.matrix(X2$Sigma))
r3<-cbind(r3,rep(as.numeric(0), nrow(r3)))
r4<-cbind(r4,rep(as.numeric(1), nrow(r4)))
# combine the two data sets into a single matrix 
XT<-rbind(r3,r4)

xft<-data.frame(X1=XT[,1],X2=XT[,2], X3=XT[,3],X4=XT[,4],src=as.factor(XT[,5]))
...
levels(ub_y)<-c(0,1)
xfd_ub_smt<-ubSMOTE(xfd[,1:4],ub_y, perc.over = 0.5*100*(nrow(r1)/nrow(r2)), k = 5, perc.under = 100, verbose = TRUE)
...
xfd_rose<-data.frame(xfd_rose$data[,1:4],"src"=as.factor(xfd_rose$data[,5]))
